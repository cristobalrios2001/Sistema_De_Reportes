...
const Visita: React.FC = () => {
    ...
    return(
        <div className="flex min-h-screen relative w-screen items-center bg-zinc-300 bg-cover text-xl">
          ...
          <div className="BarraIZQ absolute rounded-xl bg-[#384E55] ml-2 h-128 w-64 shadow-lg backdrop-blur-md max-sm:px-8 text-center">
            ...
            <div className="flex flex-col">
              <div className="flex flex-col mb-16">
                <button className="mt-7 bg-[#384E55] rounded-3xl px-10 py-2 text-white backdrop-blur-md transition-colors duration-300 hover:bg-[#47636b]" onClick={() => router.push('profile')}>Reportes</button>
              </div>
              <button className="mt-64 bg-[#384E55] rounded-3xl px-10 py-2 text-white backdrop-blur-md transition-colors duration-300 hover:bg-[#47636b]" onClick={() => router.push('login')}>Cerrar Sesión</button>
            </div>
          </div>
          ...
          <div className='DropDown flex flex-col w-[240px] h-[340py] text-black absolute inset-x-0 top-0 ml-80 mt-20'>
            <button className='bg-white px-8 w-full flex flex-col items-center justify-between ml-16 rounded' onClick={() => setIsOpen((prev) => !prev)}>
              ...
              <div className='bg-white absolute top-8 item-start rounded w-full z-10'>
                  {rajos.map((item, i) => (
                      <button className='backdrop-blur-md transition-colors duration-300 hover:bg-gray-300 w-full'>
                        <h3>
                          {item.nombre}
                        </h3>
                      </button>
